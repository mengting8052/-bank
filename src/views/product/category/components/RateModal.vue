<template>
  <Modal v-model="show" :title="$t('product.category.chooseTaxInfo')" width="700">
    <Form inline :label-width="80" ref="searchForm" :model="form">
      <FormItem :label="$t('product.category.area')">
        <Cascader :data="nations" v-model="area" change-on-select @on-change="areaChange"></Cascader>
      </FormItem>
      <FormItem :label="$t('product.category.gvtCode')" prop="gvtCode">
        <Input type="text" v-model="form.gvtCode"></Input>
      </FormItem>
      <FormItem :label="$t('product.category.hscode')" prop="hscode">
        <Input type="text" v-model="form.hscode"></Input>
      </FormItem>
      <FormItem :label="$t('product.category.taxName')" prop="name">
    ...
    return {
      ...
      table: {
        ...
        columns: [
          ...
          {
            title: i18n.t("product.category.taxName"),
            key: "name"
          },
        ]
      },
